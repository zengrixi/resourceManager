 ...
 *
 * 需要提供 QPixmap 的背景图和九宫格的 4 个变宽来创建 NinePatchPainter 对象，绘图的接口很简单，只有 2 个参数，QPainter
 * 和 QRect， 调用 NinePatchPainter.paint(painter, rect) 就使用了九宫格的方式绘图，不需要其他的操作.
 */
...
public:
     ...
     */
    NinePatchPainter(const QPixmap &background, int left, int top, int right, int bottom, bool horizontalStretch = true,
                     bool verticalStretch = true);
    ~NinePatchPainter();
