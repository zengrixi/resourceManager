...
NXDockWidget::NXDockWidget(const QString& title)
    : QDockWidget(nullptr), m_area(Qt::NoDockWidgetArea), m_state(DockWidgetState::Unknown) {
...
NXDockWidget::~NXDockWidget() { qDebug() << "NXDockWidget dtor"; }

QString NXDockWidget::windowTitle() const { return m_titleWidget->getText(); }
...
void NXDockWidget::updateTopLevelState(bool topLevel) {
    ...
    if (topLevel) {
        ...
        std::for_each(m_tabifieds.begin(), m_tabifieds.end(),
                      [&](NXDockWidget* dockWidget) { dockWidget->removeFromTabifiedDocks(this); });
        ...
        QList<QDockWidget*> tabifiedDockWidgetList =
            static_cast<NXMainWindow*>(parentWidget())->tabifiedDockWidgets(this);
        tabifiedDockWidgetList.push_back(this);

        std::for_each(std::begin(tabifiedDockWidgetList), std::end(tabifiedDockWidgetList),
...
void NXDockWidget::setWidget(QWidget* widget) { m_layout->addWidget(widget); }

void NXDockWidget::setTabifiedDocks(const QList<QDockWidget*>& dockWidgetList) {
    m_tabifieds.clear();

    std::transform(std::begin(dockWidgetList), std::end(dockWidgetList), std::back_inserter(m_tabifieds),
                   [&](QDockWidget* qDockWidget) { return static_cast<NXDockWidget*>(qDockWidget); });
}
