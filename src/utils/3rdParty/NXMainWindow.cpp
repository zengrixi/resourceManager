...
void NXMainWindow::adjustDockWidget(NXDockWidget* dockWidget) {
    ...
    switch (dockWidget->getArea()) {
        ...
        case Qt::RightDockWidgetArea: {
            dockWidget->setGeometry(rect.left() + rect.width() - dockWidget->width(), rect.top(), dockWidget->width(),
                                    rect.height());
        } break;

        case Qt::BottomDockWidgetArea: {
            dockWidget->setGeometry(rect.left(), rect.top() + rect.height() - dockWidget->height(), rect.width(),
                                    dockWidget->height());
        } break;
    }
}
...
std::list<NXDockWidget*> NXMainWindow::getDockWidgetListAtArea(Qt::DockWidgetArea area) {
    ...
    std::copy_if(
        std::begin(m_dockWidgets), std::end(m_dockWidgets), std::back_inserter(dockWidgetList),
        [area](const NXDockWidget* dockWidget) { return (dockWidget->getArea() == area) && (dockWidget->isDocked()); });
...
QRect NXMainWindow::getDockWidgetsAreaRect() {
    ...
    std::for_each(std::begin(leftAreaDockWidgets), std::end(leftAreaDockWidgets),
                  [&left](const NXDockWidget* dockWidget) {
    ...
    std::for_each(std::begin(rightAreaDockWidgets), std::end(rightAreaDockWidgets),
                  [&right](const NXDockWidget* dockWidget) {
                      if ((dockWidget->x() >= 0) && (dockWidget->width() > 0)) {
    ...
    std::for_each(std::begin(bottomAreaDockWidgets), std::end(bottomAreaDockWidgets),
                  [&bottom](const NXDockWidget* dockWidget) {
                      if ((dockWidget->y() >= 0) && (dockWidget->height() > 0)) {
...
void NXMainWindow::menuWindows_triggered(QAction* action) {
    auto it = std::find_if(m_dockWidgets.begin(), m_dockWidgets.end(),
                           [&](NXDockWidget* dockWidget) { return (dockWidget->getMenuAction() == action); });
    if (it == m_dockWidgets.end()) {
        return;
    }
