...
NXDockWidgetTitle::NXDockWidgetTitle()
    : QFrame(nullptr), m_LMPressed(false), m_autoHideEnabled(false), m_textLabel(nullptr) {
...
NXDockWidgetTitle::~NXDockWidgetTitle() { qDebug() << "NXDockWidgetTitle dtor"; }
...
void NXDockWidgetTitle::mouseMoveEvent(QMouseEvent* event) {
    if (m_LMPressed) {
        ...
        if (dockWidget != nullptr) {
            ...
            event = new QMouseEvent(QEvent::MouseButtonPress, event->pos(), mapToGlobal(event->pos()), Qt::LeftButton,
...
void NXDockWidgetTitle::setFloating(bool state) { m_autoHideButton->setVisible(state); }
